        ...
        return runCatching {
            val content = client.chat {
                ...
                system(buildString {
                    ...
                    appendLine("{\"game\": \"游戏名称\", \"title\": \"曲目标题\"}")
                    ...
                    appendLine("title是曲目标题。如果图中没有曲目标题，那么title一项为\"\"；")
                    appendLine("你的回复必须严格使用以下 json 格式：")
                    appendLine("{\"results\": [{\"game\": \"maimai\", \"title\": \"曲目标题\"}]}")
        ...
        return runCatching {
            val content = client.chat {
                ...
                system(buildString {
                    ...
                    appendLine("对于每张图片，首先判断是否是音游成绩图，如果不是则不要加入结果列表；")
                    appendLine("如果是音游成绩图，那么在列表中对应位置加入一项，格式参考如下：")
                    appendLine("{\"game\": \"游戏名称\", \"title\": \"曲目标题\", \"achievement\": \"100.3249%\", \"difficulty\": \"谱面难度\", \"combo\": \"fc\", \"sync\": \"fs\"}")
                    appendLine("game一项为游戏名称。对于maimai/舞萌DX，这一项为maimai；对于chunithm/中二节奏，这一项为chunithm；其他音游则不加入结果列表。")
                    appendLine("game一项只允许出现列表中的值：[\"maimai\", \"chunithm\"]，其他音游不得加入结果列表。")
                    appendLine("title一项为曲目标题，achievement一项为达成率。")
                    ...
                    appendLine("你的回复必须严格使用以下 json 格式：")
                    appendLine("{\"results\": [{\"game\": \"maimai\", \"title\": \"曲目标题\", \"achievement\": \"100.3249%\", \"difficulty\": \"Master\", \"combo\": \"fcp\", \"sync\": \"fsd\"}]}")
                    appendLine("其中 results 是一个数组，数组长度必须小于等于传入的图片数量。")
                })
                user {
                    images.forEach { img ->
