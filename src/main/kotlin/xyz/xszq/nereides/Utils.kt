...
suspend fun VfsFile.imageType(): String = withContext(Dispatchers.IO) {
    val stream = ImageIO.createImageInputStream(File(absolutePath).inputStream())
    val iter: Iterator<*> = ImageIO.getImageReaders(stream)
    val reader: ImageReader = iter.next() as ImageReader
