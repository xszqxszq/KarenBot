...
import xyz.xszq.bot.rhythmgame.maimai.MaimaiUtils.toDXId
import xyz.xszq.bot.rhythmgame.maimai.payload.ChartStat
...
class GuessGame(
    ...
    private val aliases: MaimaiAliases
) {
            ...
            launch {
                GlobalEventChannel.channel.takeWhile { !finished }.collect { e ->
                    if (e is BotRestartEvent) {
                        reply("bot即将重启，已暂时终止游戏，为您带来不便深感抱歉。本轮答案如下：" + musics.getInfo(music.id))
                        finished = true
                ...
                GlobalEventChannel.channel.takeWhile { !finished }.collect { e ->
                    kotlin.runCatching {
                        if (e is BotRestartEvent) {
                            finished = true
                            started[contextId] = false
                            reply("bot即将重启，已暂时终止游戏，为您带来不便深感抱歉。本轮答案如下：")
                            reply(showOpeningStatus(nowMusics, nowChars, true))
                            return@collect
                        }
                        if (e !is PublicMessageEvent ||
                            e.contextId != contextId)
