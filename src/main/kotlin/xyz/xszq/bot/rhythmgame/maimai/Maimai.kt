...
import xyz.xszq.bot.rhythmgame.maimai.MaimaiUtils.dailyOps
import xyz.xszq.bot.rhythmgame.maimai.MaimaiUtils.difficulties
import xyz.xszq.bot.rhythmgame.maimai.MaimaiUtils.getPlateByFilename
...
import xyz.xszq.bot.rhythmgame.maimai.MaimaiUtils.name2Difficulty
import xyz.xszq.bot.rhythmgame.maimai.MaimaiUtils.plateCategories
import xyz.xszq.bot.rhythmgame.maimai.MaimaiUtils.recordCategories
import xyz.xszq.bot.rhythmgame.maimai.MaimaiUtils.versionsBrief
import xyz.xszq.bot.rhythmgame.maimai.payload.ChartNotes
import xyz.xszq.bot.rhythmgame.maimai.payload.PlateResponse
import xyz.xszq.bot.rhythmgame.maimai.payload.PlayerData
...
object Maimai {
    ...
    private suspend fun updateBindings(openId: String, type: String, credential: String) = transactionWithLock {
        ProberBinding.findById(openId) ?.let {
            it.type = type
            it.credential = credential
        } ?: run {
            ProberBinding.new(openId) {
                this.type = type
    ...
    private suspend fun getCredential(arg: String, event: MessageEvent): Pair<String, String>? = event.run {
        if (arg.isNotBlank()) Pair("username", arg)
            ...
            reply(ListArk.build {
                ...
                text { "请使用使用“/mai bind 用户名或qq号”来绑定(此绑定与查分器绑定无关，在查分器绑定之后仍需在Bot这里绑定一次)" }
                text { "" }
        ...
        when (status) {
            HttpStatusCode.BadRequest ->
                reply(ListArk.build {
                    desc { "maimai功能说明" }
                    prompt { "maimai" }
                    text { "绑定的账号/指定的用户名不存在，请确认用户名对应的玩家在 Diving-Fish 的舞萌 DX 查分器上已注册" }
                    text { "请使用使用“/mai bind 用户名或qq号”来绑定（这个绑定与查分器上面的绑定无关！！！无！关！）" }
                    text { "" }
                    text { "（常见问题：为什么需要再在机器人这里绑定一次QQ号？答：因为机器人获取不到QQ号！！获取不到！！）" }
                    link("https://otmdb.cn/jump/maimaidxprober") { "点我进入查分器" }
                })
        ...
        GlobalEventChannel.subscribePublicMessages("/mai", permName = "maimai") {
            startsWith("bind") { raw ->
                ...
                if (args.isEmpty()) {
                    reply(ListArk.build {
                        desc { "maimai绑定说明" }
                        prompt { "maimai绑定" }
                        text { "在使用maimai功能前，您需要在机器人处绑定您的查分器账号。" }
                        text { "使用方法（以下两种二选一，建议绑qq）：" }
                        text { "/mai bind qq号。例：/mai bind 123456" }
                        text { "/mai bind 用户名。例：/mai bind maxscore" }
                        text { "也可以使用“/mai bind qq qq号”或者“/mai bind username 用户名”来指名绑定的类型。" }
                        text { "" }
                        text { "（常见问题：为什么需要再在机器人这里绑定一次QQ号？答：因为机器人获取不到QQ号！！获取不到！！）" }
                        link("https://otmdb.cn/jump/maimaidxprober") { "点我进入查分器" }
                    })
            ...
            startsWith(listOf("b50", "/b50")) { arg ->
                val (type, credential) =
                    if (arg.isNotBlank()) Pair("username", arg)
                        ...
                        reply(ListArk.build {
                            desc { "b50功能说明" }
                            prompt { "b50" }
                            text { "您未在本机器人绑定账号（这个绑定与查分器上面的绑定无关！！！无！关！）" }
                            text { "请使用“/mai bind 用户名或qq号”来绑定！" }
                            text { "或者使用“/b50 用户名”来按用户名查询" }
                            text { "" }
                            text { "（常见问题：为什么需要再在机器人这里绑定一次QQ号？答：因为机器人获取不到QQ号！！获取不到！！）" }
                            link("https://otmdb.cn/jump/maimaidxprober") { "点我进入查分器" }
                        })
                ...
                when (status) {
                    ...
                    HttpStatusCode.BadRequest -> {
                        if (credential.startsWith("bind")) {
                            reply("笑死，你把“/mai bind”打成了“/b50 bind”，哈哈哈哈哈")
                            return@startsWith
                        }
                        reply(ListArk.build {
                            desc { "b50功能说明" }
                            prompt { "b50" }
                            text { "您的QQ未在机器人处绑定查分器账号（详见/mai bind）或所查询的用户名不存在，" +
                ...
                val (type, credential) =
                    if (arg.isNotBlank()) Pair("username", arg)
                        ...
                        reply(ListArk.build {
                            ...
                            text { "您未在本机器人绑定账号（这个绑定与查分器上面的绑定无关！！！无！关！）" }
                            text { "请使用“/mai bind 用户名或qq号”来绑定！" }
                            text { "或者使用“/mai mb50 用户名”来按用户名查询" }
                            text { "" }
                            text { "（常见问题：为什么需要再在机器人这里绑定一次QQ号？答：因为机器人获取不到QQ号！！获取不到！！）" }
                            link("https://otmdb.cn/jump/maimaidxprober") { "点我进入查分器" }
                        })
                ...
                val (type, credential) =
                    if (arg.isNotBlank()) Pair("username", arg)
                        ...
                        reply(ListArk.build {
                            ...
                            text { "您未在本机器人绑定账号（这个绑定与查分器上面的绑定无关！！！无！关！）" }
                            text { "请使用“/mai bind 用户名或qq号”来绑定！" }
                            text { "或者使用“/mai ap50 用户名”来按用户名查询" }
                            text { "" }
                            text { "（常见问题：为什么需要再在机器人这里绑定一次QQ号？答：因为机器人获取不到QQ号！！获取不到！！）" }
                            link("https://otmdb.cn/jump/maimaidxprober") { "点我进入查分器" }
                        })
            ...
            startsWith("info") { name ->
                val music = name.ifBlank { return@startsWith }
                val data = getVersionData("all", "",this) ?: return@startsWith
                if (musics.any { it.id == music }) {
                    ...
                    musics.filter {
                        it.basicInfo.title.toDBC().lowercase() == music.toDBC().lowercase()
                    }.firstOrNull() ?.let {
