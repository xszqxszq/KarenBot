...
class MaimaiImage(
    ...
    suspend fun load(themeName: String) {
        ...
        theme.b50.pos.filterNot { it.value.fontName.isBlank() }.forEach {
            fonts[it.value.fontName] =
                globalFontRegistry.locateFontByName(it.value.fontName) ?: globalFontRegistry.defaultFont()
        }
        ...
        return result.modify {
            kotlin.runCatching {
                ...
                drawCharts(PlayScore.fillEmpty(info.charts["sd"]!!, 35), config.oldCols,
                    config.pos.getValue("oldCharts").point, config)
                drawCharts(PlayScore.fillEmpty(info.charts["dx"]!!, 15), config.newCols,
                    config.pos.getValue("newCharts").point, config)
    ...
    private suspend fun Context2d.drawCharts(charts: List<PlayScore>, cols: Int, pos: Point,
    ...
    companion object {
        var fonts = ConcurrentHashMap<String, TtfFont>()
    }
}