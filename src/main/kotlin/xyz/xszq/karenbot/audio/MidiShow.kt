...
import xyz.xszq.karenbot.GroupCommandWithArg
import xyz.xszq.karenbot.api.availableUA
import xyz.xszq.karenbot.kotlin.trimLiteralTrident
import xyz.xszq.karenbot.mirai.quoteReply
import xyz.xszq.karenbot.text.TextSettings
        ...
        events.subscribeGroupMessages {
            finding(Regex(TextSettings.data.values["regex_midishow"]!!)) { finding ->
                finding.groupValues.lastOrNull { i -> i.isNotBlank() }?.let { keyword ->
                    search.checkAndRun(this, keyword)
                }
            }
        }
    }

    val search = GroupCommandWithArg("midi搜索", "search") { keyword ->
