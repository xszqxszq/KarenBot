...
import xyz.xszq.karenbot.*
import xyz.xszq.karenbot.api.BilibiliApi
import xyz.xszq.karenbot.api.CookiesResponse
import xyz.xszq.karenbot.api.RemoteBotResponse
import xyz.xszq.karenbot.image.ImageHandler
    ...
    override suspend fun subscribe() {
        events.subscribeAlways<BotOnlineEvent> {
            GlobalScope.launch {
                ...
                KarenBot.cookies = response.data!!.cookies
                KarenBot.bkn = response.data!!.bkn
            }
        }
...
//    }
    val lightApp = CommandWithType<GroupMessageEvent>("小程序解析", "lightapp") {
        ...
        KarenBot.json.decodeFromString<LightAppRoot>(raw.content).meta.values.forEach { app ->
            if (app.title == "哔哩哔哩") {
                runCatching {
