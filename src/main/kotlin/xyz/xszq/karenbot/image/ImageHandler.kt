...
import xyz.xszq.karenbot.Cooldown
...
import xyz.xszq.karenbot.kotlin.tempDir
import xyz.xszq.karenbot.mirai.getTmpId
import xyz.xszq.karenbot.mirai.queryUrl
import xyz.xszq.karenbot.mirai.quoteReply
import xyz.xszq.karenbot.text.AutoReplyHandler
...
class ReplyPicList {
    ...
    fun load(dir: String, target: String = dir) {
        ...
        if (!KarenBot.configFolder.resolve("image").exists())
            KarenBot.configFolder.resolve("image").mkdir()
        if (!KarenBot.configFolder.resolve("image/$dir").exists())
            KarenBot.configFolder.resolve("image/$dir").mkdir()
        Files.walk(KarenBot.configFolder.resolve("image/$dir").toPath(), 2)
...
//        return imgFile
    val headers: List<Pair<String, String>> =
        if (KarenBot.cookies.isBlank()) listOf()
        else listOf(Pair("Cookie", KarenBot.cookies))
