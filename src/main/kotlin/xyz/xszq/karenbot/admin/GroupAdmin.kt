...
import xyz.xszq.karenbot.CommonCommandWithArgOf
        ...
        events.subscribeGroupMessages {
            startsWithSimple("功能列表") { moduleName, _ ->
                ...
                if (moduleName.isBlank()) {
                    ...
                    listText += KarenBot.modules.filter { it.name.isNotBlank() }
                        .joinToString("，") { it.name }
                    listText += "。\n如需查询模块下包含哪些功能，请输入“功能列表 模块名”。"
                    quoteReply(listText)
                } else {
                    KarenBot.modules.filter { it.name.isNotBlank() }.find { it.name.lowercase() == moduleName } ?.let { m ->
            ...
            startsWithSimple("启用功能") { funcName, _ ->
                if (!sender.isAdminCommandPermitted())
                    return@startsWithSimple
                KarenBot.modules.filter { it.name.isNotBlank() }.find { it.name.lowercase() == funcName } ?.let { m ->
                    ...
                    KarenBot.modules.filter { it.name.isNotBlank() }.map { it.getCommands() }.flatten()
